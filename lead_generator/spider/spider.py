...
while True:
    ...
    try:
        ...
        for cycle_count in open("spider_cycle.txt"):
        ...
        if (os.path.isfile("test_leads/lead.json") == True):
            os.remove("test_leads/lead.json")
        if (os.path.isfile("test_leads/out_lead.json") == True):
            os.remove("test_leads/lead.json")
        ...
        if current_cycle == "5":
            with open('spider_cycle.txt', 'w') as text_file:  
                text_file.write("0")
            exit
